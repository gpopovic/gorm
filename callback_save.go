...
func saveAssociationCheck(scope *Scope, field *Field) (autoUpdate bool, autoCreate bool, saveReference bool, r *Relationship) {
	...
	if scope.changeableField(field) && !field.IsBlank && !field.IsIgnored {
		...
		if r = field.Relationship; r != nil {
			...
			if value, ok := field.TagSettingsGet("SAVE_ASSOCIATIONS"); ok {
				autoUpdate = checkTruth(value)
				autoCreate = autoUpdate
				saveReference = autoUpdate
			} else if value, ok := scope.Get("gorm:save_associations"); ok {
				autoUpdate = checkTruth(value)
				autoCreate = autoUpdate
				saveReference = autoUpdate
			}

			if value, ok := field.TagSettingsGet("ASSOCIATION_AUTOUPDATE"); ok {
				autoUpdate = checkTruth(value)
			} else if value, ok := scope.Get("gorm:association_autoupdate"); ok {
				autoUpdate = checkTruth(value)
			}

			if value, ok := field.TagSettingsGet("ASSOCIATION_AUTOCREATE"); ok {
				autoCreate = checkTruth(value)
			} else if value, ok := scope.Get("gorm:association_autocreate"); ok {
				autoCreate = checkTruth(value)
			}

			if value, ok := field.TagSettingsGet("ASSOCIATION_SAVE_REFERENCE"); ok {
				saveReference = checkTruth(value)
			} else if value, ok := scope.Get("gorm:association_save_reference"); ok {
				saveReference = checkTruth(value)
			}
		}
	}
